...
class _CampScreenState extends State<CampScreen>
 ...
 Future<void> _loadAssetsAndInitialize() async {
   ...
   final List<SoldierData.Soldier> hordeMembers =
       List.from(gameState.horde);
       ...
       if (soldier.state == SoldierState.walking) {
         for (int j = 0; j < _soldiers.length; j++) {
           ...
           if ((soldier.position - otherSoldier.position).distance *
             ...
             soldier.waitTimer =
                 _random.nextDouble() * 15.0 + 8.0; // 8-23 sec
             break;
           }
         }
     ...
     body: Stack(
       children: [
         ...
         ...yurts.map((yurt) {
           ...
           double topPos =
               position.dy * screenSize.height - currentYurtHeight;
