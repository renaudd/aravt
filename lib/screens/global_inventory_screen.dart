 ...
 Widget _buildItemTile(InventoryItem item, {bool isDragging = false}) {
   return Container(
     ...
     child: ListTile(
       ...
       subtitle: Text(
         "${item.itemType.name} | ${item.quality ?? 'Standard'}",
...
class _CommunalInventoryTabState extends State<CommunalInventoryTab> {
                   ...
                   child: DataTable(
                     ...
                     columns: [
                       ...
                       DataColumn(
                           ...
                           numeric: true,
                           onSort: (i, asc) => _sort<num>((e) => e.weightPerUnit,
                               i, asc, ledgerEntries)),
                       DataColumn(
 ...
 List<_LedgerEntry> _generateLedger(GameState gameState) {
   ...
   if (gameState.communalArrows > 0) {
     ...
     entries.add(_LedgerEntry.resource(Icons.arrow_upward, Colors.white70,
         "Arrows (Standard)", gameState.communalArrows.toDouble(), 0.05, 0, 0.5));
   }
