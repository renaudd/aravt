...
// import '../models/horde_data.dart'; // FIX: No longer needed for navigation


class YurtDetailScreen extends StatelessWidget {
 // FIX 1: The constructor now only takes the Yurt's ID
 final String yurtId;
 ...
 Widget build(BuildContext context) {
   // FIX 2: Get all state from the provider
   final gameState = context.watch<GameState>();
   final List<Soldier> allSoldiers = gameState.horde;


   // FIX 3: Look up the yurt using the yurtId
   final Yurt yurt;
   try {
   ...
   // FIX 4: Get the occupants list using the helper function
   final List<Soldier> yurtOccupants = _getOccupants(yurt, allSoldiers);


   // FIX 5: 'currentHorde' object is no longer needed for navigation
                       ...
                       itemBuilder: (context, index) {
                         ...
                         return ListTile(
                           leading: GestureDetector(
                             onTap: () {
                               // FIX 6: Use the correct, refactored constructor
                               Navigator.push(
                                 context,
                                 MaterialPageRoute(
...
List<Soldier> _getOccupants(Yurt yurt, List<Soldier> allSoldiers) {
 return yurt.occupantIds
     .map((id) => allSoldiers.firstWhere((s) => s.id == id,
         orElse: () => _createUnknownSoldier(id, yurt.id))) // Find soldier by ID
     .toList();
}
...
Soldier _createUnknownSoldier(int id, String yurtId) {
 ...
 DateTime defaultBirthDate = DateTime(SoldierGenerator.gameStartYear - 25, 1, 1);
 Soldier unknown = Soldier(
     ...
     fungibleRupees: 20,
     fungibleScrap: 39,
     // --- REFACTORED: Removed old inventory lists ---
     // horses: [],
     // bows: [],
     // swords: [],
     // spears: [],
