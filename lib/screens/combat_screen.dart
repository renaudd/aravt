...
class _CombatScreenState extends State<CombatScreen>
  ...
  Future<void> _loadAllAssets() async {
    ...
    try {
      ...
      for (String color in allColors) {
        ...
        loadingFutures.add(_loadImage(
                'assets/images/sprites/${color}_horse_archer_spritesheet.png')
            .then((img) {
          if (mounted) _spritesheets['${color}_horse_archer'] = img;
        }));
        loadingFutures.add(_loadImage(
        ...
        if (['red', 'blue', 'purple', 'teal'].contains(color)) {
          loadingFutures.add(_loadImage(
                  'assets/images/sprites/${color}_spearman_spritesheet.png')
              .then((img) {
            if (mounted) _spritesheets['${color}_spearman'] = img;
          }).catchError((e) {
            return null;
          }));
          loadingFutures.add(_loadImage(
                  'assets/images/sprites/${color}_spearman_spritesheet_right.png')
              .then((img) {
            if (mounted) _spritesheets['${color}_spearman_right'] = img;
          }).catchError((e) {
            return null;
          }));
        }
  ...
  void _startSimulationTimer() {
    ...
    int delayMs = (secondsPerTurn *
        ...
        .round();
    delayMs = delayMs.clamp(10, 2000); // Reduced min delay for faster speed
              ...
              child: SizedBox(
                ...
                child: AnimatedBuilder(
                  ...
                  builder: (context, child) {
                    final currentCombatants = context
                            .watch<GameState>()
                            .currentCombat
                            ?.allCombatSoldiers ??
                        [];
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      TextButton(
                        child: Text(
                            '${gameState.combatSpeedMultiplier.toInt()}x',
                            style: GoogleFonts.cinzel(color: Colors.white)),
                        onPressed: () {
                          final gs = context.read<GameState>();
                          double next = (gs.combatSpeedMultiplier == 1.0)
                              ? 2.0
                              : (gs.combatSpeedMultiplier == 2.0 ? 4.0 : 1.0);
                      ...
                      IconButton(
                        ...
                        color: Colors.white,
                        onPressed: () =>
                            context.read<GameState>().skipCombatToEnd(),
                      ),
                    ],
                  ),
            ...
            child: Container(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  else if (event.severity == EventSeverity.low)
                    eventColor = Colors.grey[400]!;
                  return Text(
                      "[${event.date.toShortString()}] ${event.message}",
                      style:
                          GoogleFonts.cinzel(color: eventColor, fontSize: 12));
                },
              ),
            ),
          ),
  ...
  Widget _buildPostCombatPanel(BuildContext context, GameState gameState) {
    final report = gameState.latestCombatReport;
    if (report == null) {
      return Scaffold(
        backgroundColor: Colors.black,
        body: Center(
...
class BattlefieldPainter extends CustomPainter {
  ...
  final List<GameEvent> events; // Added events
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final soldier in combatants) {
      ...
      if (sheet == null) {
        ...
        canvas.drawCircle(Offset(soldier.x, soldier.y), 5.0,
            paint); // Slightly larger fallback circle
        continue;
      }
      ...
      final Rect srcRect = Rect.fromLTWH(
          srcLeft,
          0,
          _CombatScreenState.frameWidth.toDouble(),
          _CombatScreenState.frameHeight.toDouble());
      final Rect dstRect = Rect.fromCenter(
          center: Offset(soldier.x, soldier.y),
          width: baseSpriteRenderSize,
          height: baseSpriteRenderSize);
...
class BackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double worldHeight =
        simulator?.battlefield.length.toDouble() ?? 500.0;

    if (image == null) {
      canvas.drawRect(Rect.fromLTWH(0, 0, worldWidth, worldHeight),
    ...
    final Rect sourceSubrect =
        Alignment.center.inscribe(fittedSizes.source, srcRect);
    final Rect destinationSubrect =
