...
class TutorialService extends ChangeNotifier {
 ...
 bool _isPermanentlyDismissed = false;


 bool get isActive => _isActive;
 // [GEMINI-FIX] This getter MUST be here for the overlay to see the count
 ...
 final List<TutorialStepData> _steps = [
   ...
   TutorialStepData(
       ...
       requiredRoute: '/roster'), // Assuming '/roster' or '/horde' is your roster screen route
 ...
 void startTutorial(BuildContext context) {
   if (_isPermanentlyDismissed) return;
  
   _isActive = true;
   _currentIndex = 0;
  
   // Ensure we start at the right place physically and visually
   _checkAndNavigate(context, _steps[0].requiredRoute);
   notifyListeners();
 }
 ...
 void dismiss(GameState gameState) {
   ...
   if (captain != null) {
       ...
       captain.getRelationship(gameState.player?.id ?? -1).updateAdmiration(-0.4);
       captain.getRelationship(gameState.player?.id ?? -1).updateRespect(-0.2);
     
       // 2. Check for permanent dismissal (3 strikes)
       if (_dismissalCount >= 3) {
            _isPermanentlyDismissed = true;
            gameState.logEvent(
 ...
 void complete(GameState gameState, {bool success = false}) {
   ...
   if (success) {
       ...
       if (captain != null) {
           ...
           captain.getRelationship(gameState.player?.id ?? -1).updateAdmiration(0.5);
           captain.getRelationship(gameState.player?.id ?? -1).updateRespect(0.2);
