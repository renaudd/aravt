...
class _ProtoSoldier {
 ...
 static DateTime _generateDateOfBirth(Random random, int age) {
   ...
   return DateTime(gameStartYear - age, random.nextInt(12) + 1,
       random.nextInt(28) + 1);
 }
...
class GameSetupService {
 ...
 void _debugCheckLeader(String stage, List<dynamic> soldiers) {
   ...
   try {
     if (soldiers is List<Soldier>) {
       leader =
           soldiers.firstWhere((s) => s.role == SoldierRole.hordeLeader);
       print(
 ...
 GameState createNewGame(
   ...
   final List<Soldier> playerHorde = _finalizeSoldierList(protoHorde,
       isPlayerHorde: true, hasHorse: true);
 ...
 List<String> _assignRolesAndAravtsPlayer(
   ...
   try {
     ...
     print("Critical: Player not found in proto horde during role assignment.");
   }
 ...
 NpcHorde _createNpcHorde({
   ...
   List<Soldier> finalHorde = _finalizeSoldierList(protoHorde,
       isPlayerHorde: false, hasHorse: true);
 ...
 List<String> _assignRolesAntiMeritocratic(
   ...
   List<_ProtoSoldier> candidates = List.from(protoHorde);
   candidates.sort((a, b) =>
       a.combinedAntiMeritScore.compareTo(b.combinedAntiMeritScore));
   return _assignStandardNpcRoles(candidates, idPrefix);
 }


 List<String> _assignStandardNpcRoles(
     List<_ProtoSoldier> all, String prefix) {
   ...
   for (final area in worldMap.values) {
     ...
     for (final poi in settlementPois) {
       ...
       for (int i = 0; i < soldierCount; i++) {
         protoSoldiers.add(
             _ProtoSoldier(_globalSoldierIdCounter++, false, _random));
       }


       protoSoldiers
           .sort((a, b) => a.dateOfBirth.compareTo(b.dateOfBirth));
         ...
         for (var s in finalUnit) {
           if (!s.equippedItems.containsKey(EquipmentSlot.spear)) {
             try {
               ...
               if (spear != null)
                 s.equippedItems[EquipmentSlot.spear] = spear;
             } catch (e) {}
           }
         }
