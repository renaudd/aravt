...
class SoldierUpdateService {
  ...
  void _processDailyUpdate(Soldier soldier, GameDate currentDate, GameState gameState) {
    // Check for birthday
    ...
    // TODO: Add daily natural healing (for non-combat injuries/ailments)
    // TODO: Add daily experience gain from assignments
  }

  /// Applies all effects of a soldier's birthday.
  void _applyBirthdayAging(Soldier soldier, GameDate currentDate, GameState gameState) {
    soldier.age++;
  ...
  void _recalculateMaxHealth(Soldier soldier) {
    ...
    if (newMaxHealth != soldier.healthMax) {
      ...
      soldier.headHealthCurrent = soldier.headHealthCurrent.clamp(0, soldier.headHealthMax);
      soldier.bodyHealthCurrent = soldier.bodyHealthCurrent.clamp(0, soldier.bodyHealthMax);
      soldier.rightArmHealthCurrent = soldier.rightArmHealthCurrent.clamp(0, soldier.rightArmHealthMax);
      soldier.leftArmHealthCurrent = soldier.leftArmHealthCurrent.clamp(0, soldier.leftArmHealthMax);
      soldier.rightLegHealthCurrent = soldier.rightLegHealthCurrent.clamp(0, soldier.rightLegHealthMax);
      soldier.leftLegHealthCurrent = soldier.leftLegHealthCurrent.clamp(0, soldier.leftLegHealthMax);
    }
  }
