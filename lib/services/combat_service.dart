...
class BattlefieldState {
 ...
 BattlefieldState() {
   grid = List.generate(
       width, (x) => List.generate(length, (y) => BattlefieldTile(x: x, y: y)));
   _generateTerrain();
 }
...
class CombatSoldier {
 ...
 double get currentMovementSpeed {
   ...
   bool legInjured = soldier.startingInjury ==
           StartingInjuryType.limpRightLeg ||
 ...
 void takeDamage(HitLocation loc, int amt) {
   ...
   if (!isAlive && (headHealthCurrent <= 0 || bodyHealthCurrent <= 0))
     print(
         "Soldier ${soldier.id} potentially killed or knocked out...");
 }
...
class CombatSimulator {
 ...
 final List<String> _availableNpcColors =
     List.from(CombatSimulator.npcColors);
 ...
 void _initializeUnits(
     List<Aravt> aravts, int teamId, List<Soldier> pool) {
   const double edgeBufferX = 50.0;
   const double edgeBufferY = 50.0;


   final double team0DeploymentLineX = edgeBufferX + 50.0;
   final double team1DeploymentLineX =
       _battlefield.width - edgeBufferX - 50.0;
   ...
   for (var aravt in aravts) {
     ...
     final soldiersInAravt =
         pool.where((s) => s.aravt == aravt.id).toList();
     ...
     for (var soldier in soldiersInAravt) {
       ...
       double unitY =
           (aravtBaseY + _random.nextDouble() * verticalSpread - verticalSpread / 2)
               .clamp(startYMin, startYMax);
       double unitX =
           (startX + _random.nextDouble() * horizontalSpread - horizontalSpread / 2)
 ...
 void _calculateInitiative() {
   ...
   if (_captainsMap.isEmpty) {
     print("CRITICAL: No captains in _captainsMap for initiative calculation!");
     return;
   }
 ...
 Offset _determineMovementGoal(CombatSoldier captain,
     CombatSoldier? targetEnemy, CombatTactic tactic) {
 ...
 bool _runAravtFlightCheck(
     List<CombatSoldier> members, EngagementRule rule) {
   int fleeingCount = 0;
   int aliveCount = 0;
   ...
   for (var member in members) {
     if (member.isAlive) {
       if (member.isFleeing ||
           _runIndividualFlightCheck(member, rule, aliveCount, members.length)) {
         if (!member.isFleeing) {
 ...
 bool _runIndividualFlightCheck(CombatSoldier soldier, EngagementRule rule,
   ...
   double currentHealth = (soldier.headHealthCurrent +
       ...
       .toDouble();
   double injuryFactor = (MAX_HEALTH_ESTIMATE - currentHealth)
           .clamp(0.0, MAX_HEALTH_ESTIMATE) /
       MAX_HEALTH_ESTIMATE;


   bool hasSevereWound =
       soldier.injuriesSustained.any((i) => i.severity >= 3);
   ...
   double stressFactor =
       ((soldier.currentStress - 1.0) / 4.0).clamp(0.0, 1.0);
   ...
   double finalThreshold = (FLEE_BASE_THRESHOLD - courageBonus - leadershipBonus)
       .clamp(0.25, 0.95);
 ...
 void _executeMove(CombatSoldier soldier, double targetX, double targetY) {
   ...
   Offset midPoint = currentPos + moveDirection * (distanceToMoveAttempt * 0.5);
   ...
   switch (tactic) {
     case CombatTactic.maintainDistance:
       if (longBow != null &&
         ...
         _logAttack(
             attacker, "throws spear defensively at", target, spear!);
     ...
     case CombatTactic.chargeSpears:
       if (distance <= 5.0) {
         if (spear != null &&
           ...
           _logAttack(attacker, "charges with ${melee.name} against", target,
               melee);
     ...
     case CombatTactic.takeCover:
       if (longBow != null &&
           ...
           !_runExhaustionCheck(attacker, ItemType.bow, false)) {
         _logAttack(
             attacker, "fires long bow from cover at", target, longBow);
         _resolveRangedAttack(attacker, target, longBow);
         attacker.arrowsRemaining--;
         didAttack = true;
       }
 ...
 void _logAttack(CombatSoldier attacker, String action, CombatSoldier target,
     Weapon weapon) {
   _logMessage(
       "${attacker.soldier.name} $action ${target.soldier.name}!",
       isInternal: false,
 ...
 void _resolveRangedAttack(
   ...
   if (target.isAlive && targetShield != null && targetShield.condition > 0) {
     ...
     if (roll >= requiredRoll) {
       _logMessage(
           "${target.soldier.name} blocks the shot with their shield!",
           isInternal: false,
           severity: EventSeverity.low,
           soldierId: target.soldier.id);
   ...
   _resolveDamageAndInjury(
       attacker, target, damage, weapon.damageType, weapon,
       blockedByShield: false);
 }
 ...
 void _resolveDamageAndInjury(CombatSoldier attacker, CombatSoldier target,
   ...
   bool shieldCouldCover = (hitLocation == HitLocation.body ||
       hitLocation == HitLocation.leftArm);
   if (!blockedByShield &&
       shield != null &&
       shield.condition > 0 &&
       shieldCouldCover) {
     ...
     _logMessage(
         "The blocked hit impacts ${target.soldier.name}'s shield!",
         isInternal: false,
         severity: EventSeverity.low,
         soldierId: target.soldier.id);
     if (penetratingDamage <= shield.deflectValue)
   ...
   if (!deflected && armor != null && armor.condition > 0) {
     ...
     _logMessage(
         "Direct hit on ${target.soldier.name}'s ${hitLocation.name}!",
         isInternal: false,
         severity: EventSeverity.normal,
         soldierId: target.soldier.id);
   }
   ...
   if (injury != null) {
     ...
     if (minorHpDamage > 0) {
       ...
       if (!target.isAlive &&
           (target.headHealthCurrent <= 0 || target.bodyHealthCurrent <= 0)) {
         _killSoldier(target, "minor wounds", _gameState,
             attacker: attacker);
       }
     } else {
   ...
   if (item.condition == 0) {
     ...
     try {
       ...
       if (owner != null && slot != null) {
         ...
         _logMessage(
             "${owner.soldier.name}'s ${item.name} is now unequipped.",
             isInternal: true,
             soldierId: owner.soldier.id);
       }
     } catch (e) {
       print(
 ...
 void _applyExhaustion(CombatSoldier soldier, double baseCost) {
   ...
   if (newExhaustion >= 5.0 &&
       soldier.isAlive &&
       !soldier.isUnconscious) {
     soldier.isUnconscious = true;
     soldier.headHealthCurrent = max(1, soldier.headHealthCurrent);
     _logMessage(
         "${soldier.soldier.name} collapses from exhaustion!",
         isInternal: false,
         severity: EventSeverity.critical,
         soldierId: soldier.soldier.id);
   }
 ...
 bool _runDodgeCheck(CombatSoldier target, CombatSoldier attacker,
   ...
   double exhaustionPenalty =
       max(0, target.currentExhaustion - 2.0) * 0.01;
 ...
 bool _runParryCheck(
   ...
   double exhaustionPenalty =
       max(0, target.currentExhaustion - 2.0) * 0.02;
 ...
 bool _runHitCheck(
     CombatSoldier attacker, CombatSoldier target, Weapon weapon, bool isCharge,
   ...
   double finalHitChance =
       baseHitChance + skillModifier + exhaustionModifier;
   ...
   if (!success)
     _logMessage(
         "${attacker.soldier.name} misses ${target.soldier.name}.",
         isInternal: false,
         severity: EventSeverity.normal,
         isGoodNews: false,
         soldierId: attacker.soldier.id);
