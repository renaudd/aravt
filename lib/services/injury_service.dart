...
class InjuryService {
  ...
  static HitLocation determineHitLocation() {
    ...
    if (roll < 10)
      return HitLocation.head; // 10%
    else if (roll < 50)
      return HitLocation.body; // 40%
    else if (roll < 65)
      return HitLocation.leftArm; // 15%
    else if (roll < 80)
      return HitLocation.rightArm; // 15%
    else if (roll < 90)
      return HitLocation.leftLeg; // 10%
    else
      return HitLocation.rightLeg; // 10%
  }
    ...
    switch (location) {
      case HitLocation.head:
        if (severity == 4) {
          ...
          if (damageType == DamageType.Piercing) {
            name = "Arrow/Spear Eye/Temple";
            hpMin = 4;
            hpMax = 7;
            bleed = 0.5;
            unconscious = true;
          } else if (damageType == DamageType.Slashing) {
            name = "Skull Fracture (Open)";
            hpMin = 3;
            hpMax = 6;
            bleed = 1.0;
            unconscious = true;
          ...
          if (damageType == DamageType.Piercing) {
            name = "Lost Eye/Punctured Cheek";
            hpMin = 2;
            hpMax = 4;
            bleed = 0.3;
          } else if (damageType == DamageType.Slashing) {
            name = "Lost Ear/Deep Facial Cut";
            hpMin = 1;
            hpMax = 3;
            bleed = 0.2;
          } else {
            name = "Broken Jaw/Mod Concussion";
            hpMin = 2;
            hpMax = 4;
            stun = 1;
          }
      ...
      case HitLocation.body:
        if (severity == 4) {
          ...
          hpMax = 10;
          unconscious = true;
          limbLoss = true; // "Limb loss" here means instant death pretty much
        } else if (severity == 3) {
          if (damageType == DamageType.Piercing) {
            name = "Punctured Lung/Liver";
            hpMin = 4;
            hpMax = 7;
            bleed = 1.0;
            unconscious = _random.nextDouble() < 0.5;
          } else if (damageType == DamageType.Slashing) {
            name = "Disemboweled";
            hpMin = 4;
            hpMax = 7;
            ...
            name = "Crushed Chest/Spleen Rupture";
            hpMin = 5;
            hpMax = 8;
            bleed = 0.8;
            unconscious = true;
          ...
          if (damageType == DamageType.Piercing) {
            name = "Minor Organ Puncture";
            hpMin = 2;
            hpMax = 4;
            bleed = 0.4;
          } else if (damageType == DamageType.Slashing) {
            name = "Deep Flesh Cut (Torso)";
            hpMin = 1;
            hpMax = 3;
            bleed = 0.3;
          } else {
      ...
      case HitLocation.rightArm:
        if (severity == 4) {
          ...
          bleed = 2.0;
          limbLoss = true;
          unconscious = _random.nextDouble() < 0.3;
        } else if (severity == 3) {
          if (damageType == DamageType.Piercing) {
            name = "Impaled Bone (Arm)";
            hpMin = 4;
            hpMax = 7;
            bleed = 0.6;
            stun = 1;
          } else if (damageType == DamageType.Slashing) {
            name = "Severed Tendons (Arm)";
            hpMin = 3;
            hpMax = 6;
            bleed = 1.2;
          } else {
            name = "Shattered Bone (Arm)";
            hpMin = 4;
            hpMax = 7;
            stun = 2;
          }
          ...
          if (damageType == DamageType.Piercing) {
            name = "Impaled Flesh (Arm)";
            hpMin = 2;
            hpMax = 4;
            bleed = 0.2;
          } else if (damageType == DamageType.Slashing) {
            name = "Deep Muscle Cut (Arm)";
            hpMin = 1;
            hpMax = 3;
            bleed = 0.3;
          } else {
            name = "Fractured Bone (Arm)";
            hpMin = 1;
            hpMax = 3;
            stun = 1;
          }
      ...
      case HitLocation.rightLeg:
        if (severity == 4) {
          ...
          if (damageType == DamageType.Piercing) {
            name = "Impaled Bone (Leg)";
            hpMin = 4;
            hpMax = 7;
            bleed = 0.7;
            stun = 1;
          } else if (damageType == DamageType.Slashing) {
            name = "Severed Tendons (Leg)";
            hpMin = 3;
            hpMax = 6;
            bleed = 1.3;
          } else {
            name = "Shattered Bone (Leg)";
            hpMin = 4;
            hpMax = 7;
            stun = 2;
          }
          ...
          if (damageType == DamageType.Piercing) {
            name = "Impaled Flesh (Leg)";
            hpMin = 2;
            hpMax = 4;
            bleed = 0.2;
          } else if (damageType == DamageType.Slashing) {
            name = "Deep Muscle Cut (Leg)";
            hpMin = 1;
            hpMax = 3;
            bleed = 0.3;
          } else {
            name = "Fractured Bone (Leg)";
            hpMin = 1;
            hpMax = 3;
            stun = 1;
          }
